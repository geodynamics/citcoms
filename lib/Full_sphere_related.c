  ...
  lev = E->mesh.levmax;
	 ...
	 for (j=0, ns=step*lnoy*k; j<lvnox; j++, ns+=step) {
	   ...
	   for (i=1; i<=lvnoz; i++) {
	     ...
	     E->SX[lev][2][node] = fi;
	     E->SX[lev][3][node] = rfac * E->sphere.R[lev][i];
	     
	     /*   x,y,and z oordinates   */
	     E->X[lev][1][node] = E->data.ra * E->sphere.R[lev][i]*sint*cosf;
	     E->X[lev][2][node] = E->data.ra * E->sphere.R[lev][i]*sint*sinf;
	     E->X[lev][3][node] = E->data.rc * E->sphere.R[lev][i]*cost;
